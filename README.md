...
Note: Duplicate your local `shinytest` baselines to the other testing folder suffix values. Ex: `mytest-expected-macOS` to `mytest-expected-Ubuntu` and `mytest-expected-Windows`.  This will allow for GitHub Actions to fail and post the expected images for each platform.  If no baselines are present, shinytest will disclare the run a success and move `mytest-current` to `mytest-expected-SUFFIX`.

To see a live example of testing multiple applications on all 3 platforms and multiple R versions, take a look at [`rstudio/shinycoreci-apps`](https://github.com/rstudio/shinycoreci-apps/). In this repo, we have [`ci-runtests.yml`](https://github.com/rstudio/shinycoreci-apps/blob/master/.github/workflows/ci-runtests.yml) which will test on a all platforms and 4 different versions on a schedule.  Because there would be too many artifacts to maintain manually, failed runs are posted to automatically created branches. These autogenerated branches are deleted after a week without a new commit.  `rstudio/shinycoreci-apps` is still in flux and subject to change while the Shiny Team narrows down how to consistently test multiple applications on multiple GitHub Actions executions.
